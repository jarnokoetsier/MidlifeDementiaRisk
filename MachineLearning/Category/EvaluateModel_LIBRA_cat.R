...
for (i in 1:length(methods)){
  ...
  temp$Class[(ObsPred_CV_HighRisk$pred == "High") &
               (ObsPred_CV_LowRisk$pred == "Intermediate_High")] <- "High Risk (2-12.7)"
  temp$Class[(ObsPred_CV_LowRisk$pred == "Low") &
               (ObsPred_CV_HighRisk$pred == "Low_Intermediate")] <- "Low Risk (-2.7-0)"
  ...
  temp$Class[(ObsPred_test_HighRisk$pred == "High") &
               (ObsPred_test_LowRisk$pred == "Intermediate_High")] <- "High Risk (2-12.7)"
  temp$Class[(ObsPred_test_LowRisk$pred == "Low") &
               (ObsPred_test_HighRisk$pred == "Low_Intermediate")] <- "Low Risk (-2.7-0)"
  temp$Method <- rep(methodNames[i], nrow(temp))
...
p <- ggplot(ObsPred_CV_all) +
  ...
  ggtitle("LIBRA",subtitle = "Performance in cross-validation") +
...
p <- ggplot(ObsPred_test_all) +
  ...
  xlab("Predicted Class") +
  ylab("LIBRA Score") +
  ggtitle("LIBRA",subtitle = "Performance in test set") +
