...
ggsave(p, file = "CAIDE2_Cat_Cor_AUC_CV.png", height = 5, width = 7.5)
...
ggsave(p, file = "CAIDE2_Cat_Cor_AUC_test.png", height = 5, width = 7.5)
...
for (i in 1:length(methods)){
  ...
  temp <- data.frame(Score = ObsPred_CV_HighRisk$ObservedScore,
                     Class = rep("Intermediate Risk (5-8)",nrow(ObsPred_CV_HighRisk)))
  temp$Class[(ObsPred_CV_HighRisk$pred == "High") &
               (ObsPred_CV_LowRisk$pred == "Intermediate_High")] <- "High Risk (9-17)"
  temp$Class[(ObsPred_CV_LowRisk$pred == "Low") &
               (ObsPred_CV_HighRisk$pred == "Low_Intermediate")] <- "Low Risk (0-4)"
  ...
  temp <- data.frame(Score = ObsPred_test_HighRisk$ObservedScore,
                     Class = rep("Intermediate Risk (5-8)",nrow(ObsPred_test_HighRisk)))
  temp$Class[(ObsPred_test_HighRisk$pred == "High") &
               (ObsPred_test_LowRisk$pred == "Intermediate_High")] <- "High Risk (9-17)"
  temp$Class[(ObsPred_test_LowRisk$pred == "Low") &
               (ObsPred_test_HighRisk$pred == "Low_Intermediate")] <- "Low Risk (0-4)"
  temp$Method <- rep(methodNames[i], nrow(temp))
...
ObsPred_CV_all$Class <- factor(ObsPred_CV_all$Class,
                               levels = c("Low Risk (0-4)","Intermediate Risk (5-8)","High Risk (9-17)"))

ObsPred_CV_all$Method <- factor(ObsPred_CV_all$Method,
                                levels = methodNames)
p <- ggplot(ObsPred_CV_all) +
  geom_rect(ymin = -Inf, ymax = 5, xmin = -Inf, xmax = Inf,fill = "#FFF5F0", alpha = 0.5) +
  geom_rect(ymin = 5, ymax = 8, xmin = -Inf, xmax = Inf,fill = "#FEE0D2", alpha = 0.5) +
  geom_rect(ymin = 8, ymax = Inf, xmin = -Inf, xmax = Inf,fill = "#FCBBA1", alpha = 0.5) +
  geom_hline(yintercept = 5, linewidth = 1.5, linetype = "dashed", color = "black") +
  geom_hline(yintercept = 8, linewidth = 1.5, linetype = "dashed", color = "black") +
  ...
  ylab("CAIDE2 Score") +
  scale_y_continuous(breaks = c(0,2,4,6,8,10,12,14,16), labels = c(0,2,4,6,8,10,12,14,16))+
  ggtitle("CAIDE2") +
...
ggsave(p, file = "CAIDE2_Cat_Cor_boxPlot_CV.png", height = 5, width = 7.5)
...
ggsave(p, file = "CAIDE2_Cat_Cor_boxPlot_test.png", height = 5, width = 7.5)

model <- "ElasticNet"
test <- ObsPred_test_all[ObsPred_test_all$Method == model,]
test$obs <- rep("Intermediate Risk (5-8)", nrow(test))
test$obs[test$Score < 5] <- "Low Risk (0-4)"
test$obs[test$Score > 8] <- "High Risk (9-17)"
...
ggsave(p, file = "CAIDE2_Cat_Cor_whichFactors_test.png", height = 6, width = 10)
