...
Y_train$Y <- factor(ifelse(Y_train$Diagnosis == "NL","Control","Cognitive_Impaired"),
                    levels = c("Control", "Cognitive_Impaired"))

Y_test$Y <- factor(ifelse(Y_test$Diagnosis == "NL","Control","Cognitive_Impaired"),
                    levels = c("Control", "Cognitive_Impaired"))
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_CI_EN.RData")
...
roc_test <- roc(Y_test$Y, testPred$Cognitive_Impaired)
auc(roc_test)
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_CI_sPLS.RData")
...
# Save model
save(fit, file = "EMIF/Fit_EMIF_CI_RF.RData")
...
load("EMIF/Fit_EMIF_CI_RF.RData")
RF <- predict(fit, X_test, type = "prob")
load("EMIF/Fit_EMIF_CI_EN.RData")
EN <- predict(fit, X_test, type = "prob")
load("EMIF/Fit_EMIF_CI_sPLS.RData")
...
testDF <- data.frame(EN = EN$Cognitive_Impaired,
                     sPLS = sPLS$Cognitive_Impaired,
                     RF = RF$Cognitive_Impaired,
                     EpiCAIDE1 = EpiCAIDE1,
                     EpiLIBRA = EpiLIBRA,
                     EpiAge = X_test$Age,
                     Age = Y_test$Age,
...
ggsave(p, file = "EMIF/ROC_CI_EMIF.png", width = 8, height = 5)
