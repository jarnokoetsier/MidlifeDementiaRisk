...
# Get samples with PGS and CAIDE1 score
samples_train <- Y_LIBRA[Y_LIBRA$ID %in% rownames(PGS),]
PGS_train <- t(PGS[samples_train$ID,])
colnames(PGS_train) <- samples_train$Basename
...
# Make training data
X_train <- X_train[,samples_train$Basename]
...
for (i in 1:length(CVindex)){
  ...
  # Actual training
  test <- rbind(X_train[finalProbes,], PGS_train)
  set.seed(123)
  fit <- train(x = t(test),
               y = Y_train$LIBRA,
...
save(trainResults, file = "PGS/trainResults_LIBRA_CorPGS_EN.RData")
...
test <- rbind(X_train[,samples_train$Basename], PGS_train)
Y_train =  samples_train
all(colnames(test) == Y_train$Basename)

# Actual training
set.seed(123)
fitControl <- fitControl <- trainControl(method = "none")
finalModel <- train(x = t(test),
                    y = Y_train$LIBRA,
                    metric= "RMSE",
                    method = "glmnet",
                    tuneGrid = data.frame(
...
save(trainResults, optLambda, optAlpha, perf, finalModel,
     file = "PGS/CV_LIBRA_CorPGS_EN.RData")
...
RMSE(pred = test, obs = samples_test$LIBRA)
R2(pred = test, obs = samples_test$LIBRA)
...
save(trainResults, file = "PGS/trainResults_LIBRA_CorPGS_sPLS.RData")
...
save(trainResults, optK, optEta, optKappa, perf, finalModel,
     file ="PGS/CV_LIBRA_CorPGS_sPLS.RData")
...
save(trainResults, file = "PGS/trainResults_LIBRA_CorPGS_RF.RData")
...
save(trainResults, opt_mtry, opt_splitrule, opt_min.node.size, perf, finalModel,
     file ="PGS/CV_LIBRA_CorPGS_RF.RData")
