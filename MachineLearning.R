...
EN_regression <- function(trainingData, trainingClass, nfold, nrep, alpha, lambda){
  
  # Empty matrix to collect model performance: RMSE
  rmse <- rep(NA,nrep*nfold)
  ...
  for(r in 1:nrep){
    ...
    for (i in 1:nfold){
      ...
      rmse[count] <- RMSE(pred = pred[,1], obs = Y_val)
  ...
  output <- list(rmse, coeffs, alpha, lambda)
  names(output) <- c("RMSE", "Coefficients", "Alpha", "Lambda")
  return(output)
}
