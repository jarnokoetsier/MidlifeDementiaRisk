...
Y_train <- Y_train[!is.na(Y_train$Y),]
X_train <- X_train[Y_train$X,]
...
score <- c("EN", "sPLS", "RF")
scoreName <- c("ElasticNet", "sPLS", "Random Forest")
...
plotAUC <- data.frame(AUC = paste0("AUC: ",aucValue),
                      Score = scoreName,
                      X = 0.9,
                      Y = rev(seq(0.05,0.19,length.out = length(aucValue))))
...
scoreName <- c("CSF (EN)", "CSF (sPLS)", "CSF (RF)",
               "CSF + MRS (EN)", "CSF + MRS (sPLS)", "CSF + MRS (RF)")
plotDF <- as.data.frame(testDF)
ROCplot <- NULL
aucValue <- rep(NA, length(score))
for (i in 1:length(score)){
